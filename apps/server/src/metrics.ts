...
export async function metricsRoutes(app: FastifyInstance): Promise<void> {
  app.get("/metrics", async (request, reply) => {
    ...
    const lines: string[] = [];
    lines.push("# HELP omnideck_node_up Whether the node is reporting within its configured offline threshold.");
    lines.push("# TYPE omnideck_node_up gauge");
    ...
    for (const node of nodes) {
      ...
      lines.push(`omnideck_node_up{${labels}} ${online ? 1 : 0}`);
      lines.push(`omnideck_node_cpu_percent{${labels}} ${number(node.cpu_percent)}`);
      lines.push(`omnideck_node_memory_total_bytes{${labels}} ${number(node.memory_total_bytes)}`);
      lines.push(`omnideck_node_memory_used_bytes{${labels}} ${number(node.memory_used_bytes)}`);
      lines.push(`omnideck_node_uptime_seconds{${labels}} ${number(node.uptime_seconds)}`);
      try {
        ...
        for (const disk of disks) {
          ...
          lines.push(`omnideck_node_disk_total_bytes{${diskLabels}} ${number(disk.totalBytes)}`);
          lines.push(`omnideck_node_disk_used_bytes{${diskLabels}} ${number(disk.usedBytes)}`);
        }
      } catch { /* Agent payloads are validated before storage. */ }
    }

    lines.push("# HELP omnideck_endpoint_up Whether the latest endpoint probe succeeded.");
    lines.push("# TYPE omnideck_endpoint_up gauge");
    ...
    for (const endpoint of endpoints) {
      ...
      lines.push(`omnideck_endpoint_up{${labels}} ${number(endpoint.success)}`);
      lines.push(`omnideck_endpoint_status_code{${labels}} ${number(endpoint.status_code)}`);
      lines.push(`omnideck_endpoint_ttfb_seconds{${labels}} ${number(endpoint.ttfb_ms) / 1000}`);
      lines.push(`omnideck_endpoint_duration_seconds{${labels}} ${number(endpoint.total_ms) / 1000}`);
    }

    lines.push("# HELP omnideck_ai_up Whether the latest AI streaming probe succeeded.");
    lines.push("# TYPE omnideck_ai_up gauge");
    ...
    for (const target of targets) {
      ...
      lines.push(`omnideck_ai_up{${labels}} ${number(target.success)}`);
      lines.push(`omnideck_ai_status_code{${labels}} ${number(target.status_code)}`);
      lines.push(`omnideck_ai_ttft_seconds{${labels}} ${number(target.ttfb_ms) / 1000}`);
      lines.push(`omnideck_ai_duration_seconds{${labels}} ${number(target.total_ms) / 1000}`);
      if (target.balance !== null) lines.push(`omnideck_ai_balance{${labels}} ${number(target.balance)}`);
    }

    const openAlerts = one<{ count: number }>(db.prepare("SELECT COUNT(*) AS count FROM alerts WHERE status='open'"))?.count ?? 0;
    lines.push("# HELP omnideck_alerts_open Number of currently open alerts.");
    lines.push("# TYPE omnideck_alerts_open gauge");
    lines.push(`omnideck_alerts_open ${openAlerts}`);
